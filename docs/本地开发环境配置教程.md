...
数据库名配置: [IntelliForum-web/src/main/resources/application.yml](../IntelliForum-web/src/main/resources/application.yml)
...
本项目中所有使用的表定义放在 [liquibase](../IntelliForum-web/src/main/resources/liquibase)
...
首先在进入之前，先简单了解一下配置，当前所有的配置放在`IntelliForum-web`模块内，我们做了环境区分，
...
接下来以默认的dev环境配置为例，首先进入配置文件 [application-dal.yml](../IntelliForum-web/src/main/resources-env/dev/application-dal.yml)
...
进入启动类: [QuickForumApplication](../IntelliForum-web/src/main/java/com/walnut/web/QuickForumApplication.java)
